...
export function SolutionParticleSimulator() {
  ...
  // Hydration-safe: start with stable empty array on server, populate on client
  const [particles, setParticles] = useState<Particle[]>([]);

  useEffect(() => {
    setParticles(makeParticles(Math.min(solute, 50)));
  }, [solute]);

  const total = solute + water;
  const concentration = (solute / total) * 100;
  const fillOpacity = 0.3 + (concentration / 100) * 0.4;
  const waterFraction = Math.min(1, water / 100);
    ...
    <div className="rounded-2xl bg-white dark:bg-zinc-900 p-6 shadow-sm border border-zinc-200 dark:border-zinc-800 space-y-5">
      ...
      <div className="rounded-xl bg-zinc-50 dark:bg-zinc-950/40 border border-zinc-200 dark:border-zinc-800 p-4 flex justify-center">
        <svg viewBox="0 0 240 220" className="w-full max-w-[280px]">
          ...
          <rect
            x="32"
            y={30 + (180 - 180 * waterFraction)}
            width="176"
            height={180 * waterFraction}
            fill={`rgba(96,165,250,${fillOpacity.toFixed(2)})`}
            rx="2"
          />
          {particles.map((p) => (
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1">
          <div className="text-xs font-medium text-zinc-700 dark:text-zinc-300">용질(설탕) {solute}g</div>
          <input
            type="range"
            min={0}
            max={50}
        ...
        <div className="space-y-1">
          <div className="text-xs font-medium text-zinc-700 dark:text-zinc-300">물(용매) {water}g</div>
          <input
            type="range"
            min={50}
            max={200}
