# 완료 감사 기준

`npm run audit:completion`은 저장소에서 자동으로 확인 가능한 “완성도 차단 이슈”를 검사한다.

## 현재 결과

2026-05-22 기준:

| 항목 | 결과 |
|------|------|
| 마스터 인덱스 ID | 392 |
| 앱 등록 단원 | 392 |
| 활성 단원 | 392 |
| planned 단원 | 0 |
| generated stub page | 0 |
| 패턴엔진 | 20/20 |
| legacy renderer | 0 |
| 공통 학습자료 | 392/392 |
| 세부 UnitContent | 392/392 |
| UnitBlueprint | 392/392 |
| 488 확장 후보 | 96 보류 |
| 차단 이슈 | 0 |

## 검사 항목

- `docs/00-MASTER-INDEX.md`에 열거된 모든 ID가 앱 메타데이터에 등록되어 있는지 확인
- 앱 메타데이터의 모든 ID가 마스터 인덱스에도 존재하는지 확인
- 모든 등록 단원이 `planned`가 아닌 상태인지 확인
- 모든 등록 단원의 route page가 존재하는지 확인
- 생성용 placeholder/stub 문구가 남아 있지 않은지 확인
- 모든 활성 단원 페이지가 `InteractiveErrorBoundary`를 사용하는지 확인
- 모든 활성 단원의 `componentName`이 `components/interactive`에서 export되는지 확인
- 공개 저장소 기본 문서가 존재하는지 확인

`npm run audit:content`는 교육자료 필수 구성을 별도로 검사한다.

- 등록 단원 수와 `UnitContent` 수가 공식 검증 목표 수(현재 392)와 일치하는지 확인
- 앱에 등록되지 않은 단원 ID가 `UnitContent`에 섞이지 않았는지 확인
- 모든 등록 단원에 `sourceRefs`가 2개 이상 있고 문서 또는 URL이 있는지 확인
- 모든 등록 단원에 쉬운/표준/심화 설명이 있는지 확인
- 모든 등록 단원에 예시가 2개 이상 있는지 확인
- 모든 등록 단원에 미니 문제가 정확히 3문항인지 확인
- 미니 문제 순서가 개념 확인, 적용 문제, 실수 교정 또는 전이인지 확인
- 모든 미니 문제에 `answer`와 `explanation`이 분리되어 있는지 확인
- 모든 `nextUnitIds`가 현재 단원 레지스트리에 존재하는지 확인
- 모든 등록 단원에 핵심질문이 있는지 확인
- 모든 등록 단원에 3개 학습목표가 있는지 확인
- 모든 등록 단원에 `See -> Touch -> Predict -> Explain -> Challenge` 5단계 루프가 있는지 확인
- 모든 등록 단원에 미니 도전, 오개념 교정, 적용 장면, 산출물, 3개 복습 질문이 있는지 확인
- 모든 단원 페이지가 공통 학습자료 표면을 렌더링하는지 확인

488개 확장 목표는 별도 정책으로 관리한다. 추가 96개는 NCIC, 고교학점제 지원센터 등 공식 출처에서 단원명·과목·학교급·영역·출처가 확인되기 전까지 앱 데이터와 감사 목표 수에 포함하지 않는다.

## 실행

```bash
npm run audit:completion
npm run audit:content
npm run audit:blueprint
npm run audit:interaction
```

JSON 결과가 필요하면:

```bash
npm run audit:completion -- --json
```

## CI 연동

완료 감사는 GitHub Actions의 `CI` workflow에 포함되어 있다. 단원 ID, 라우트, 상태, 컴포넌트 export, 공개 문서 중 하나라도 어긋나면 PR이 실패한다.

## UnitBlueprint Audits

`npm run audit:blueprint` checks the generated `UnitBlueprint` registry for the 392 verified units. It fails if a unit is missing blueprint data, source locators, 1200+ characters of authored material, 3 examples, 3 mini problems, answer explanations, common mistakes, real-life applications, or valid `nextUnitIds`.

`npm run audit:interaction` checks the pattern-engine contract. It fails if a unit lacks a known engine ID, variables, initial state, feedback rules, misconception responses, acceptance criteria, 44px touch targets, or 360px mobile readiness.

The 96 remaining units toward the 488 target remain blocked until official per-unit source rows are recorded.

## 범위

이 감사는 저장소 구조와 코드 연결성을 검증한다. 교육학적 최종 감수, Figma 최종 시안 승인, 접근성 전문 감사처럼 사람의 판단이 필요한 항목은 별도 리뷰 기준으로 다룬다.
