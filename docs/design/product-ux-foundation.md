...
|------------|---------|----|
| Learn | 개념의 핵심 질문과 출처 기반 세부 설명 제공 | 3분 훑어보기, 쉬운/표준/심화 설명, 핵심 예시 |
| Manipulate | 변수를 직접 바꾸고 현상을 관찰 | 인터랙티브 캔버스, 슬라이더, 드래그, 토글 |
| Check | 이해 여부를 짧게 확인 | 고정 3문항 미니 문제, 정답과 해설 |
| Review | 복습 큐와 다음 경로로 연결 | 흔한 실수, 실생활 적용, 자기평가 버튼, 다음 단원 CTA |
...
브랜드 이름은 `하루배움`을 기준으로 한다. 로고와 캐릭터 자산은 `docs/design/brand-assets.md`를 따르며, 캐릭터는 오늘 큐, 복습 큐, 완료 피드백을 보조하는 밝은 학습 파트너로 사용한다.
...
├─ Learn: 3분 훑어보기, 쉬운/표준/심화 설명, 핵심 예시, 출처 refs
├─ Manipulate: 밝은 실험실형 캔버스, 조작 패널, 실시간 결과
├─ Check: 3문항 미니 문제, 정답과 해설
├─ Review: 흔한 실수, 실생활 적용, 자기평가 버튼, 다음 단원
...
현재 저장소 감사 기준은 392개 단원이다. 장기 제품 브리프에서 488개/800개 내외 리소스를 목표로 언급하더라도, README나 진행률에는 `npm run audit:completion`과 `npm run audit:content`로 확인되는 수치만 쓴다. 488개 목표 중 추가 96개는 공식 출처 행이 확인되기 전까지 앱 데이터가 아니라 후보 문서에만 둔다.
