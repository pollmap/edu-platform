...
2. **코드 반영은 출처가 있는 입력물 기준**으로 한다. Figma 링크, 캡처, token export, NCIC/공식 출처를 우선한다.
3. **문서 수치와 앱 수치는 자동 감사로 맞춘다.** 새 단원을 추가하거나 상태를 바꾸면 `npm run audit:completion`을 통과해야 한다.
4. **보안과 공개 저장소 기본 문서는 유지한다.** LICENSE, LICENSE-CONTENT, SECURITY, CONTRIBUTING, README가 누락되면 감사가 실패한다.
